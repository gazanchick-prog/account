...
CRYPTO_TOKEN = "569935:AArlMmgry9w4mlYA74SiMtxfBtK0E4FqnP2"
ADMIN_ID = 8212981789
...
async def get_acc_info(session_path):
    """Получает номер телефона аккаунта"""
...
async def get_last_codes(session_path):
    """Получает 2 последних кода и их даты"""
    client = TelegramClient(session_path, API_ID, API_HASH)
    try:
        await client.connect()
        messages = []
...
def main_kb(user_id):
    ...
    builder.row(types.InlineKeyboardButton(text="👤 Профиль", callback_data="profile"),
                types.InlineKeyboardButton(text="💳 Пополнить", callback_data="dep_choice"))
...
async def buy(c: types.CallbackQuery):
    ...
    if acc:
        ...
        await bot.send_document(c.from_user.id, acc[1], caption="✅ Успешная покупка! Аккаунт выше.\nДля получения кода перейдите в 'Мои покупки'.")
...
async def show_account_info(c: types.CallbackQuery):
    ...
    wait = await c.message.answer("⏳ Извлекаю данные номера...")
    ...
    b.row(types.InlineKeyboardButton(text="⬅️ К покупкам", callback_data="my_orders"))
    
    await wait.edit_text(f"📱 **Аккаунт:**\n\n**Номер телефона:** `{phone}`\n\nИспользуйте этот номер для входа, затем нажмите кнопку ниже, чтобы получить код.", 
...
async def view_codes_call(c: types.CallbackQuery):
    ...
    wait = await c.message.answer("⏳ Читаю последние сообщения...")
...
# --- ПОПОЛНЕНИЕ ---
@dp.callback_query(F.data == "dep_choice")
async def dep_choice(c: types.CallbackQuery):
    b = InlineKeyboardBuilder()
    b.row(types.InlineKeyboardButton(text="💎 CryptoBot", callback_data="deposit"))
    b.row(types.InlineKeyboardButton(text="👨‍💻 Поддержка", url=SUPPORT_LINK))
    await c.message.edit_text("Выберите способ:", reply_markup=b.as_markup())

@dp.callback_query(F.data == "deposit")
async def dep_amt(c: types.CallbackQuery, state: FSMContext):
    await c.message.answer("Сумма в TON:")
...
async def dep_invoice(m: types.Message, state: FSMContext):
    try:
        amt = float(m.text)
        inv = await crypto.create_invoice(asset='TON', amount=amt)
        b = InlineKeyboardBuilder()
        b.row(types.InlineKeyboardButton(text="Оплатить", url=inv.bot_invoice_url))
        b.row(types.InlineKeyboardButton(text="Проверить", callback_data=f"check_{inv.invoice_id}_{amt}"))
        await m.answer(f"Счет на {amt} TON", reply_markup=b.as_markup())
        await state.clear()
    except: 
        await m.answer("Ошибка в числе")

@dp.callback_query(F.data.startswith("check_"))
async def check_dep(c: types.CallbackQuery):
    _, iid, amt = c.data.split("_")
    invs = await crypto.get_invoices(invoice_ids=int(iid))
    if invs and invs[0].status == 'paid':
        db_query("UPDATE users SET balance = balance + ? WHERE id = ?", (amt, c.from_user.id), commit=True)
        await c.message.edit_text("✅ Пополнено!")
    else: 
